...
# select projects that use at least 10 images: 

image_per_project = sqldf("select owner_id, project_name, owner_project, count(*) as image_count from compose_file_images group by owner_project")
at_least_ten_images<-image_per_project[which(image_per_project$image_count>9),]

projects<-read.csv(here("rqs/data","root_compose_projects_commit_count_100.csv"), header=TRUE)
at_least_ten_images_repos<-sqldf("select projects.url, at_least_ten_images.owner_id, 
      at_least_ten_images.project_name,
      at_least_ten_images.image_count 
      from projects inner join at_least_ten_images
      on projects.owner_id = at_least_ten_images.owner_id
      and projects.name = at_least_ten_images.project_name ")

# select projects that use 1 image: 

image_per_project = sqldf("select owner_id, project_name, owner_project, count(*) as image_count from compose_file_images group by owner_project")
one_image<-image_per_project[which(image_per_project$image_count==1),]

projects<-read.csv(here("rqs/data","root_compose_projects_commit_count_100.csv"), header=TRUE)
one_images_repos<-sqldf("select projects.*, one_image.owner_id, 
      one_image.project_name,
      one_image.image_count 
      from projects inner join one_image
      on projects.owner_id = one_image.owner_id
      and projects.name = one_image.project_name ")

#write.csv(one_images_repos[sample(nrow(one_images_repos), 100),], "../Manual_analysis/Rq1/single_image.csv")
